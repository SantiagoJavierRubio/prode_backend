...
export const verifyEmail = async (req, res) => {
    try {
        ...
        if(!verificationToken || verificationToken.token !== token) throw new Error('Invalid token')
        if(Date.now() > token.expiration) return res.redirect('/auth/email/token-expired')
...
export const loginWithEmail = async (req, res) => {
    try {
        const user = await User.checkCredentials(req.body.email || null, req.body.password || null)
        if(user.error) throw new Error(user.error)
