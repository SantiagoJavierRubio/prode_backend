...
export class AuthController {
    ...
    async createWithEmail(req: Request, res: Response, next: NextFunction) {
        try {
            const response = await authService.createWithEmailAndSendVerification(req.body)
            res.json(response)
        }
        catch(err) {
            errorHandler(err, req, res, next)
    ...
    async verifyEmail(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            if (!token || typeof token !== 'string') throw new CustomError(404, "Token missing")
            if (!userId || typeof userId !== 'string') throw new CustomError(404, "User_id missing")
            const verified = await authService.checkVerification(token, userId)
            if (verified) res.redirect(`${config.clientUrl}/auth/verified`)
            else throw new CustomError(500, "Something went wrong with your verification")
        }
        catch(err) {
            errorHandler(err, req, res, next)
        }
    ...
    async requirePasswordChange(req: Request, res: Response, next: NextFunction) {
        try {
            const response = await authService.handlePasswordChangeRequest(req.body.email);
    ...
    async grantTemporaryVerification(req: Request, res: Response, next: NextFunction) {
        try {
