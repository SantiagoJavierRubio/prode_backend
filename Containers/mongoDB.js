class Container {
    ...
    async getById(id, fields=null) {
        try {
            const result = fields ? await this.model.findById(id, fields) : await this.model.findById(id)
    ...
    async getMany(options={}, fields=null) {
        try {
            const results = fields ? await this.model.find(options, fields) : await this.model.find(options)
    ...
    async getOne(match, fields=null) {
        try {
            const result = fields ? await this.model.findOne(match, fields) : await this.model.findOne(match)
            if(!result) return null
            return result
        } catch(err) {
            return new Error(`Failed to obtain element, error: ${err}`)
