...
const showByGroupName = async (predictions) => {
  ...
  await predictions.forEach(async (prediction) => {
    if (Object.keys(byGroups).includes(prediction.userGroupId)) {
      ...
      const groupInfo = groupNames.find(
        (group) => `${group._id}` === prediction.userGroupId
      );
...
export const predictionsByStage = async (predictions, stageId) => {
  if (hasNulls([predictions, stageId]))
  ...
  const predictionsByStage = await predictions.filter((prediction) =>
...
export const filterForOneGroup = async (predictions, groupId) => {
  if (hasNulls([predictions, groupId]))
    throw new CustomError(406, "Missing field");
...
const calculateCompletedPredictionsByStage = (predictions, stageMatches) => {
  const included = stageMatches.filter((match) =>
...
export const getPredictionQuantityForStages = async (predictions) => {
  ...
  const stageCodes = Object.fromEntries(
    stageNames.map((name) => [getStageCode(name), name])
  );
  ...
  const matchesByStage = stages.map((stage) => {
    if (stage.matches)
      return [
        stageCodes[stage.id],
        calculateCompletedPredictionsByStage(predictionMatchIds, stage.matches),
      ];
    const groupStageMatches = stage.groups.map((group) => group.matches).flat();
    return [
      stageCodes[stage.id],
      calculateCompletedPredictionsByStage(
        predictionMatchIds,
        groupStageMatches
      ),
    ];
