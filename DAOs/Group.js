...
class Group extends Container {
  ...
  async getMembers(groupName) {
    if (hasNulls([groupName]))
      throw new CustomError(406, "No group name provided");
    const group = await this.getOne({ name: groupName.toUpperCase() });
    if (!group) throw new CustomError(404, "Group not found");
  ...
  async removeMember(groupName, user_id) {
    ...
    if (group.owner === user_id) {
      ...
      const changedOwner = await this.update(group._id, {
        owner: otherMembers[0],
      });
      if (!changedOwner) throw new CustomError(500, "Failed to change admin");
    }
    const edited = await this.update(group._id, {
  ...
  async getCommonGroups(user_id, other_id) {
    if (hasNulls([user_id, other_id]))
      throw new CustomError(406, "Missing field");
    const commonGroups = await this.getMany(
      { members: { $all: [user_id, other_id] } },
      "name"
    );
    return commonGroups;
  }
