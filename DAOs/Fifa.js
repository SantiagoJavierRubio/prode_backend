...
class Fifa extends Container {
    ...
    async getAllMatches(lang='es') {
        const data = await axios.get(`${this.apiUrl}/calendar/matches?idSeason=${this.SEASON_ID}&count=100&language=${lang}`)
    ...
    async getOneStage(id, lang='es') {
        const data = await axios.get(`${this.apiUrl}calendar/matches?idSeason=${this.SEASON_ID}&idStage=${id}&count=100&language=${lang}`)
        if(!data.data?.Results) throw new CustomError(503, 'Failed to obtain fixture', 'Fifa api did not respond as expected', {response: data})
        return this.normalizeMatches(data.data.Results)
    }
    ...
    async getOneGroup(id, lang='es') {
        const data = await axios.get(`${this.apiUrl}calendar/matches?idSeason=${this.SEASON_ID}&idStage=${this.GROUP_STAGE}&idGroup=${id}&count=100&language=${lang}`)
        if(!data.data?.Results) throw new CustomError(503, 'Failed to obtain fixture', 'Fifa api did not respond as expected', {response: data})
        return this.normalizeMatches(data.data.Results)
    }
}
